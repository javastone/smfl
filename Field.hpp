...
Field::Field() :
	game_over(1),
	random_engine(static_cast<unsigned>(std::chrono::system_clock::now().time_since_epoch().count()))
{
...
void Field::draw(sf::RenderWindow& i_window)
{
	...
	std::cout << "loading font texture  11122 " << std::endl;
	if(!icons_texture.loadFromFile("files\\Images\\Icons" + std::to_string(CELL_SIZE) + ".png")){
		printf("Error loading icons texture\n");
	};
