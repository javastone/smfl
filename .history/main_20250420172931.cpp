...
int main()
{
    ...
    happyTexture.loadFromFile("files/images/face_win.png");
    ...
    while (gameWindow.isOpen())
    {
        ...
        while (gameWindow.pollEvent(event))
        {
            ...
            else if (newBoard.getStatus() == 0 && event.type == sf::Event::MouseButtonPressed)
            {
                ...
                if (happyPositionX <= mousePosition.x && mousePosition.x <= happyEndX && happyPositionX <= mousePosition.y && mousePosition.y <= happyEndY)
                {
                    debug = (!debug);
                    newBoard.reset();
                    timer.reset();
