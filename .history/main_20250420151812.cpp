...
int main()
{
    ...
    while (gameWindow.isOpen())
    {
       ...
       // Timer display positions
        ...
        // Convert elapsed time to strings
        ...
        for (char c : minutes) {
drawDigit(c, gameSprite, digitsTexture, currentX, timerY, gameWindow); // Move to the next position
        }
...
void drawDigit(char c, sf::Sprite &gameSprite, sf::Texture &digitsTexture, float &currentX, float timerY, sf::RenderWindow &gameWindow)
{
            int digit = c - '0'; // Convert char to int
            gameSprite.setTextureRect(sf::IntRect(digit * 21, 0, 21, digitsTexture.getSize().y));
            gameSprite.setPosition(currentX, timerY);
