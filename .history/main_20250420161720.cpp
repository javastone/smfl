...
int main()
{
    ...
    while (gameWindow.isOpen())
    {
        ...
        string mineCounter =(abs(remainingMines) < 100 ? "0" : "")+ (abs(remainingMines) < 10 ? "0" : "") +to_string(abs(remainingMines));

        float counterStartX = 33; // Starting X position for minute
    

        drawDigits(mineCounter, digitSprite, digitsTexture, counterStartX, digitY, gameWindow, remainingMines < 0);
...
void drawDigits(std::string &minutes, sf::Sprite &digitSprite, sf::Texture &digitsTexture, float &currentX, float positionY, sf::RenderWindow &gameWindow, bool isNegative)
{
    if (isNegative)
    {
