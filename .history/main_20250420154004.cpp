...
int main()
{
    ...
    digitsTexture.loadFromFile("files/images/digit.png");
    ...
    while (gameWindow.isOpen())
    {
        ...
        float currentX = minutesStartX;
            ...
            int digit = '9' - '0'; // Convert char to int
            gameSprite.setTextureRect(sf::IntRect(digit * 21, 0, 21, digitsTexture.getSize().y));
            gameSprite.setPosition(currentX, timerY);
            gameWindow.draw(gameSprite);
            currentX += 21; // Move to the next position

        gameSprite.setTexture(digitsTexture);
        ...
        // backgroundRect.setSize(sf::Vector2f(40, 30));
        // backgroundRect.setFillColor(sf::Color::Black);

        // backgroundRect.setPosition(numColumns * 32 - 100, (numRows + 0.5) * 32 + 10);
        ...
        // string minutes = (elapsed.first < 10) ? "0" + to_string(elapsed.first) : to_string(elapsed.first);
        // timeText.setString(minutes);
        // timeText.setPosition(numColumns * 32 - 97, (numRows + 0.5) * 32 + 16);
        // gameWindow.draw(timeText);

        // backgroundRect.setPosition(numColumns * 32 - 57, (numRows + 0.5) * 32 + 10);
        // gameWindow.draw(backgroundRect);

        // string seconds = (elapsed.second < 10) ? "0" + to_string(elapsed.second) : to_string(elapsed.second);
        // timeText.setString(seconds);
