...
public:
    ...
    {
        std::sort(board.begin(), board.end(), [](const ScoreEntry &a, const ScoreEntry &b) {
        ...
        {
            output << entry.getTimeString() + ", " + entry.name << std::endl;
        }
    }
    ...
    void addToBoard(string name, int minutes, int seconds)
    {
        ...
        if (board.size() >= 5 && board.back().totalSeconds < (totalSeconds))
        {
            return; // No need to add if the score is not in the top 5
        }
